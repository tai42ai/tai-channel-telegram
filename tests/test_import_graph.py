...
``tai_channel_telegram`` is on the allowlist. The rule (see the README): the shipped
package imports
the HTTP surface (httpx + starlette), the shared platform substrate
(tai-contract + tai-kit with the redis extra, pydantic, pydantic-settings)
...
The runtime walk imports ``tai_channel_telegram`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai_channel_telegram`` means the assertion covers the SHIPPED package's
...
PACKAGE = "tai_channel_telegram"
ALLOWED_FIRST_PARTY = frozenset({PACKAGE, "tai_contract", "tai_kit"})

# Every third-party root the shipped ``tai_channel_telegram`` graph pulls in -- the
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai_channel_telegram graph failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai_channel_telegram module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai_channel_telegram sources: {offenders}"
